...
def process(device, model, model_type, image, input_size, target_size, optimize, use_camera):
    ...
    bins = [0, 750, 1500, 2150, np.inf]
    
    # Find the bin indices using searchsorted
    bin_indices = np.searchsorted(bins, prediction)
    
    # Map the indices to the desired values
    prediction1 = np.select([bin_indices == 0, bin_indices == 1, bin_indices == 2, bin_indices == 3, bin_indices == 4],
                           [0, 750, 1500, 2150, 3000])

    # Map prediction1 based on the original prediction
    prediction2 = np.select([prediction1 == 0, prediction1 == 750, prediction1 == 1500, prediction1 == 2150, prediction1 == 3000],
                            [0, 4, 3, 2, 1])
    
    return prediction, prediction2 # prediction is distance prediction2 is depth
...
def run(input_path, output_path, model_path, model_type="dpt_beit_large_512", width1 = 8, height1 = 8, optimize=False, side=False, height=None,
    ...
    if input_path is not None:
        ...
        for index, image_name in enumerate(image_names):
            ...
            # input
            original_image_rgb = my_utils.read_image(image_name)  # in [0, 1]
            image = transform({"image": original_image_rgb})["image"]

            # compute
            with torch.no_grad():
                prediction, prediction2 = process(device, model, model_type, image, (width1, height1), (width1, height1), #wafisilo
                                     optimize, False)
            ...
            if output_path is not None:
                ...
                if not side:
                    my_utils.write_depth(filename, prediction, grayscale, bits=2)
                else:
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                if frame is not None:
                    ...
                    prediction, prediction2 = process(device, model, model_type, image, (net_w, net_h),
                                         (width1, height1), optimize, True)
...
if __name__ == "__main__":
    ...
    # compute depth maps
    modif = run(args.input_path, args.output_path, args.model_weights, args.model_type, args.optimize, args.side, args.height,
